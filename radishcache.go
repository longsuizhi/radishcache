...
// 实例化Group
func NewGroup(name string, cacacheBytes int64, getter Getter, strategy string) *Group {
	if getter == nil {
		panic("nil Getter")
	}
	mu.Lock()
	...
	g := &Group{
		name:      name,
		getter:    getter,
		mainCache: cache.New(cacacheBytes, strategy),
...
func (g *Group) getFromPeer(peer PeerGetter, key string) (cache.ByteView, error) {
	...
	return cache.ByteView{B: res.Value}, nil
}
